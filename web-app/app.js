...
function wireEvents() {
  ...
  $("#restoreInput").onchange = event => {
    const reader = new FileReader(); reader.onload = () => { state = JSON.parse(reader.result); saveState(true); renderAll(); toast("Backup restored"); }; reader.readAsText(event.target.files[0]);
  };
